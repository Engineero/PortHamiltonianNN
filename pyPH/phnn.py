...
class PHNN(nn.Module):
    ...
    def makeFMatrix(self):
            ...
            #value list
            v = torch.Tensor(np.concatenate((np.ones(n),
                                             -1 * np.ones(n),
                                             -self.beta * np.ones(n))))
    ...
    def additionalTermsLoss(self):
        return torch.add(self.hparams[1] * torch.dot(self.flat_wdot,
                                                     self.flat_wdot),
                         self.hparams[2] * torch.dot(self.flat_w,
                                                     self.flat_w))

    def assignNewState(self, xi):
        self.flat_w = torch.Tensor(xi[:len(self.flat_w)])
        self.flat_wdot = torch.Tensor(
            xi[len(self.flat_w):2 * len(self.flat_w)]
        )
    ...
    def makeStateDict(self):
        d = {}
        k = 0
        for _, key in enumerate(self.predictor.state_dict().keys()):       
                num_el = torch.numel(self.predictor.state_dict()[key])
                d[key] = self.flat_w[k:k+num_el].view(
                    self.predictor.state_dict()[key].shape
                )
                k += num_el
        return d
    ...
    def setXi(self):
        self.xi = torch.cat((self.flat_w.to(self.device),
                             self.flat_wdot.to(self.device)))
    ...
    def initializeRecord(self):
        for _ in range(len(self.flat_w)):
            self.pW.append([])
            self.pWdot.append([])
    ...
    def fit(self, trainloader, epoch=3, time_delta=1, iter_accuracy=10,
            ode_t=0.25, ode_step=10, criterion='nll'):
        """
        ...
        for _ in range(epoch): 
            for _, data in enumerate(trainloader):               
                ...
                if self.count % iter_accuracy == 0 and self.count != 0:
                    print(f'Number of odeint and parameters reassignment iterations: {self.count}')
                    print(f'In-training accuracy estimate: {self.pred_accuracy(trainloader)}')